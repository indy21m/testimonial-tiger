  ...
  create: protectedProcedure
    .input(z.object({
      ...
      config: z.object({
        ...
        triggers: z.array(z.enum(['new_testimonial', 'approved', 'featured'])),
        headers: z.record(z.string(), z.string()).optional(),
      }),
    }))
    .mutation(async ({ ctx, input }) => {
      const integration = await ctx.db.insert(integrations).values({
        userId: ctx.userId,
        type: input.type,
        name: input.name,
        config: input.config,
  ...
  update: protectedProcedure
    .input(z.object({
      ...
      config: z.object({
        webhookUrl: z.string().url().optional(),
        apiKey: z.string().optional(),
        triggers: z.array(z.enum(['new_testimonial', 'approved', 'featured'])).optional(),
        headers: z.record(z.string(), z.string()).optional(),
      }).optional(),
